...
class Graph:
    ...
    def to_networkx(self):
        ...
        # Выбор типа графа: ориентированный или неориентированный
        nx_graph = nx.DiGraph() if self.directed else nx.Graph()

        # Добавление рёбер и весов
        for u, edges in self.adjacency_list.items():
            for edge in edges:
                v = edge[0]  # Соседняя вершина
    ...
    def from_networkx(self, nx_graph):
        """Преобразует объект NetworkX в Graph."""
        self.adjacency_list = {node: [] for node in nx_graph.nodes}
        self.directed = isinstance(nx_graph, nx.DiGraph)
        self.weighted = any('weight' in data for _, _, data in nx_graph.edges(data=True))

        # Переносим рёбра и веса в adjacency_list
    ...
    def reconstruct_path(self, predecessors, start, end):
        ...
        path.reverse()
        return path if path[0] == start else []  # Если путь существует, иначе пустой список
    ...
    def dijkstra(self, start):
        ...
        while priority_queue:
            ...
            for neighbor, *weight in self.adjacency_list.get(current_vertex, []):
                weight = weight[0] if weight else 0  # Обрабатываем вес ребра (если есть)
    ...
    # Задание 10: Найти максимальный поток
    def edmonds_karp_max_flow(self, source, sink):
        source = source.strip()
        sink = sink.strip()

        if source not in self.adjacency_list or sink not in self.adjacency_list:
            raise ValueError(f"Вершины '{source}' или '{sink}' не существуют в графе.")

        if not self.directed:
            print("Алгоритм Эдмондса-Карпа применим только к ориентированным графам.")
            return None
        # Структура для хранения остаточной ёмкости
        residual_capacity = {u: {} for u in self.adjacency_list}
        for u in self.adjacency_list:
            for v, capacity in self.adjacency_list[u]:
                residual_capacity[u][v] = capacity if self.weighted else 1
                if v not in residual_capacity:
                    residual_capacity[v] = {}  # Для обратного пути
                residual_capacity[v][u] = 0  # Обратное ребро с нулевой ёмкостью

        def bfs_find_augmenting_path():
            parent = {u: None for u in self.adjacency_list}
            visited = {u: False for u in self.adjacency_list}
            queue = deque([source])
            visited[source] = True
            while queue:
                u = queue.popleft()
                for v, capacity in residual_capacity[u].items():
        ...
        while True:
            ...
            if not parent:
                break
            # Минимальная пропускная способность вдоль пути
            path_flow = float('Inf')
            v = sink
            while v != source:
                u = parent[v]
